...
def gaeproxy_handler(sock, address, hls={'setuplock':gevent.coros.Semaphore()}):
    ...
    if need_direct:
        ...
        try:
            ...
            if app_code == 503:
                logging.error('%s is over quota',common.GAE_APPIDS[0])
...
def pacserver_handler(sock, address, hls={}):
    ...
    if time.time() - hls['mtime'] > 60*60: #60*60*12
        hls['mtime'] = time.time()
...
def pre_start():
    ...
    if ctypes and os.name == 'nt':
        ...
        if common.LOVE_ENABLE:
            ...
            if time.time() - common.LOVE_TIMESTAMP > 0 and random.randint(1,10) > 0: #>86400,>5
