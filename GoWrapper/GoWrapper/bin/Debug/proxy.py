...
from random import shuffle #GFQ
...
class Common(object):
    ...
    def info(self):
        ...
        info += 'GAE Mode           : %s\n' % self.GOOGLE_MODE
        info += 'GAE Profile        : %s\n' % 'Gfanqiang Custom Profile'
        #info += 'GAE Bandwidth      : %sG\n' % len(self.GAE_APPIDS) 
...
class Autoproxy2Pac(object):
    ...
    def generate_pac(self, filename):
        ...
        if os.path.isfile('../data/proxy.gfq'):
            f = open('../data/proxy.gfq')
            lines = f.readlines()
            f.close()
            for line in lines:
                 rulelist += '\n'
...
class PACServerHandler(GAEProxyHandler):
    ...
    def first_run(self):
        #Force update PAC at the first time
        #if time.time() - os.path.getmtime(self.pacfile) > 60 * 60:
...
def pre_start():
    ...
    if ctypes and os.name == 'nt':
        ctypes.windll.kernel32.SetConsoleTitleW(u'GoAgent v%s @Gfanqiang' % __version__)
        ...
        if common.LOVE_ENABLE:
            ...
            if time.time() - common.LOVE_TIMESTAMP > 0 and random.randint(1,10) > 0:
        ...
        if softwares:
            ...
            if softwares:
                error = u'某些安全?件(如 %s)可能和本?件存在?突，造成 CPU 占用?高。\n如有此?象建???退出此安全?件????行GoAgent' % ','.join(softwares)



                ctypes.windll.user32.MessageBoxW(None, error, u'GoAgent 建?', 0)
                #sys.exit(0)
...
class gfanqiang():		
	def updateappid(self):
		while(1):
			...
			appid = response.read().split('|')
			shuffle(appid)
			response.close()
			logging.info("Gfanqiang: Updated APPID List, %sG BW Remaning",str(len(appid)))
